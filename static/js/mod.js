/**
 * Fast Roads - Ultimate Feature-Packed Mod Menu (Upgraded)
 * Toggle visibility anytime with the [ M ] key
 * Drag the header to reposition the menu.
 */
(function() {
    'use strict';
    ...
    // Populate dynamic vehicles if available
    ...
    setTimeout(() => { refreshVehicleList(); loadVehicleToUI(vehSelect.value || 'Bike'); }, 500);
    ...
    document.getElementById('fr-apply-physics').addEventListener('click', () => {
        ...
        localStorage.setItem(`fr_physics_${key}`, JSON.stringify(v.metrics));
        console.log(`[FastRoads] Physics updated & saved for ${key}`, v.metrics);
    });

    // Reset physics button
    document.getElementById('fr-reset-physics').addEventListener('click', () => {
        localStorage.removeItem(`fr_physics_${vehSelect.value}`);
        location.reload();
    });
